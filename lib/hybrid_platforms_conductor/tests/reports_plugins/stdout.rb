          ...
          out(Terminal::Table.new(headings: ['List name', '# nodes', '% tested', '% expected success', '% success']) do |table|
            nodes_by_nodes_list.each do |nodes_list, nodes_info|
              table << [
                ...
                nodes_info[:tested_nodes].empty? ? '' : "#{((100.0 - nodes_info[:tested_nodes].size + nodes_info[:tested_nodes_in_error_as_expected].size) * 100.0 / nodes_info[:tested_nodes].size).to_i} %",
