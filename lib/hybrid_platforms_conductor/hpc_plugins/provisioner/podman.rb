        ...
        def create
          ...
          if @nodes_handler.known_docker_images.include?(image)
            ...
            Futex.new(image_futex_file).open do
              @cmd_runner.run_cmd "cd #{@nodes_handler.docker_image_dir(image)} && #{podman_cmd} build --tag #{image_tag} --security-opt seccomp=/usr/share/containers/seccomp.json --cgroup-manager=cgroupfs ."
            end
