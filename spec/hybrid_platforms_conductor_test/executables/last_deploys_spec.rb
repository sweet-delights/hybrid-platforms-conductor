    ...
    with_test_platform_for_last_deploys do
      expect_ssh_executor_runs([proc do |actions, timeout: nil, concurrent: false, log_to_dir: 'run_logs', log_to_stdout: true|
        expect(actions).to eq(
          'node1' => { actions: { bash: "cd /var/log/deployments && ls -t | head -1 | xargs sed '/===== STDOUT =====/q'" } },
          'node2' => { actions: { bash: "cd /var/log/deployments && ls -t | head -1 | xargs sed '/===== STDOUT =====/q'" } }
        )
        {
          'node1' => [0, "
...
", '']
      ...
      expect(stdout).to eq(
        "+----------+---------------------+-------------+------------+------------------+-----------------+-------+\n" +
        "| Hostname | Date                | Admin       | Repository | Commit message   | Differing files | Error |\n" +
        "+----------+---------------------+-------------+------------+------------------+-----------------+-------+\n" +
...
", '']
  ...
  it 'checks the selected nodes' do
    with_test_platform_for_last_deploys do
      expect_ssh_executor_runs([proc do |actions, timeout: nil, concurrent: false, log_to_dir: 'run_logs', log_to_stdout: true|
        expect(actions).to eq('node1' => { actions: { bash: "cd /var/log/deployments && ls -t | head -1 | xargs sed '/===== STDOUT =====/q'" } })
        {
          'node1' => [0, "
date: 2019-08-21 10:12:15
user: admin_user1
