describe 'executables\' SSH Executor options' do
  ...
  it 'selects the correct gateway user' do
    with_test_platform_for_ssh_executor_options do
      ...
      exit_code, stdout, stderr = run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello', '--gateway-user', 'another_user'
  ...
  it 'selects the correct gateway conf' do
    with_test_platform_for_ssh_executor_options do
      ...
      exit_code, stdout, stderr = run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello', '--gateways-conf', 'test_gateway2'
      expect(exit_code).to eq 0
      expect(stdout).to eq ''
      expect(stderr).to eq ''
    end
  ...
  it 'does not use the SSH control master' do
    with_test_platform_for_ssh_executor_options do
      ...
      exit_code, stdout, stderr = run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello', '--no-ssh-control-master'
      expect(exit_code).to eq 0
      expect(stdout).to eq ''
      expect(stderr).to eq ''
    end
  ...
  it 'drives the maximum number of threads' do
    with_test_platform_for_ssh_executor_options do
      ...
      exit_code, stdout, stderr = run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello', '--max-threads', '5'
      expect(exit_code).to eq 0
      expect(stdout).to eq ''
      expect(stderr).to eq ''
    end
  ...
  it 'does not use strict host key checking' do
    with_test_platform_for_ssh_executor_options do
      ...
      exit_code, stdout, stderr = run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello', '--no-ssh-host-key-checking'
      expect(exit_code).to eq 0
      expect(stdout).to eq ''
      expect(stderr).to eq ''
    end
  ...
  it 'displays commands instead of running them' do
    with_test_platform_for_ssh_executor_options do
      ...
      exit_code, stdout, stderr = run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello', '--show-commands'
      expect(exit_code).to eq 0
      expect(stdout).to eq ''
      expect(stderr).to eq ''
    end
  ...
  it 'uses a different SSH user name' do
    with_test_platform_for_ssh_executor_options do
      ...
      exit_code, stdout, stderr = run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello', '--ssh-user', 'ssh_new_user'
      expect(exit_code).to eq 0
      expect(stdout).to eq ''
      expect(stderr).to eq ''
    end
  ...
  it 'fails if no user name has been given, either through environment or command-line' do
    ...
    ENV.delete 'USER'
    with_test_platform_for_ssh_executor_options do
      expect { run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello' }.to raise_error(RuntimeError, 'No SSH user name specified. Please use --ssh-user option or platforms_ssh_user environment variable to set it.')
    end
  end
  ...
  it 'expects passwords to be input' do
    with_test_platform_for_ssh_executor_options do
      ...
      exit_code, stdout, stderr = run 'ssh_run', '--host-name', 'node1', '--command', 'echo Hello', '--password'
      expect(exit_code).to eq 0
      expect(stdout).to eq ''
      expect(stderr).to eq ''
    end
