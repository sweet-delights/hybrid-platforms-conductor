describe 'ssh_config executable' do
  ...
  it 'dumps the SSH config without arguments' do
    with_test_platform_for_ssh_config do
      expect(test_ssh_executor.connector(:ssh)).to receive(:ssh_config).with(ssh_exec: 'ssh') { '# SSH config' }
  ...
  it 'dumps the SSH config with an alternate SSH executable' do
    with_test_platform_for_ssh_config do
      expect(test_ssh_executor.connector(:ssh)).to receive(:ssh_config).with(ssh_exec: 'my_ssh') { '# SSH config' }
