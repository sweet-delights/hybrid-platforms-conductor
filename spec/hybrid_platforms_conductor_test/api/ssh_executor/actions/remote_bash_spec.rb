  ...
  context 'checking actions\' plugin remote_bash' do
    ...
    it 'executes remote Bash code' do
      with_test_platform_for_action_plugins do
        test_ssh_executor.execute_actions('node' => { remote_bash: 'remote_bash_cmd.bash' })
    ...
    it 'executes remote Bash code with timeout' do
      with_test_platform_for_action_plugins do
        test_ssh_executor.connector(:test_connector).remote_bash_code = proc do |_stdout, _stderr, connector|
    ...
    it 'executes remote Bash code in several lines' do
      with_test_platform_for_action_plugins do
        test_ssh_executor.execute_actions('node' => { remote_bash: ['bash_cmd1.bash', 'bash_cmd2.bash', 'bash_cmd3.bash'] })
        expect(test_ssh_executor.connector(:test_connector).calls).to eq [
          [:connectable_nodes_from, ['node']],
          [:with_connection_to, ['node']],
    ...
    it 'executes remote Bash code using the commands syntax' do
      with_test_platform_for_action_plugins do
        test_ssh_executor.execute_actions('node' => { remote_bash: { commands: 'bash_cmd.bash' } })
        expect(test_ssh_executor.connector(:test_connector).calls).to eq [
          [:connectable_nodes_from, ['node']],
          [:with_connection_to, ['node']],
    ...
    it 'executes remote Bash code from a file' do
      with_test_platform_for_action_plugins do |repository|
        ...
        test_ssh_executor.execute_actions('node' => { remote_bash: { file: "#{repository}/commands.txt" } })
        expect(test_ssh_executor.connector(:test_connector).calls).to eq [
          [:connectable_nodes_from, ['node']],
          [:with_connection_to, ['node']],
          [:remote_bash, "bash_cmd1.bash\nbash_cmd2.bash"]
    ...
    it 'executes remote Bash code both from commands and a file' do
      with_test_platform_for_action_plugins do |repository|
        ...
        test_ssh_executor.execute_actions('node' => { remote_bash: {
