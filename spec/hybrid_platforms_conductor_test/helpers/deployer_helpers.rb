    ...
    module DeployerHelpers
      ...
      def with_platform_to_deploy(
        ...
        with_test_platform(nodes_info, !check_mode) do |repository|
          with_ssh_master_mocked_on(nodes_info[:nodes].keys) do
            packaged_times = 0
            delivered_nodes = []
      ...
      def test_deployer
        @deployer = HybridPlatformsConductor::Deployer.new logger: logger, logger_stderr: logger, cmd_runner: test_cmd_runner, nodes_handler: test_nodes_handler, ssh_executor: test_ssh_executor unless @deployer
        @deployer
      end
